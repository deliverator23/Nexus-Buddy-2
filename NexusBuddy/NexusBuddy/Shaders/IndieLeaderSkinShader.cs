...
{
	internal class IndieLeaderSkinShader : IndieMaterial
	{
		...
		[Category("Leaderhead Materials"), DisplayName("SkinBlurMap"), Editor(typeof(FilteredFileNameEditor), typeof(UITypeEditor))]
		public string SkinBlurMap
		{
			get
			{
                return ShaderUtils.trimPathFromFilename(base.GetMaterial().FindParameterSet("Civ5LeaderheadSkinTextures").GetParameterValue("SkinBlurMap") as string);
			}
			set
			{
				base.GetMaterial().FindParameterSet("Civ5LeaderheadSkinTextures").SetParameterValue("SkinBlurMap", value.Substring(value.LastIndexOf("\\") + 1));
			}
		}
		public IndieLeaderSkinShader(IGrannyMaterial material) : base(material)
		{
		}
		public void CopyTextures(string outputFolder)
		{
			...
			base.CopyTextureIfExists(this.SkinBlurMap, outputFolder);
		}
	}
}
